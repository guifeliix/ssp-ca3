...
router.get('/get/html', function(req, res) {
    ...
    let xml = fs.readFileSync('beermenu.xml', 'utf8'),
        xsl = fs.readFileSync('beermenu.xsl', 'utf8');
...
router.post('/post/json', function(req, res){
    function appendJSON(obj){
        console.log(obj);
        XMLtoJSON('beermenu.xml', function(err, result) {
            if (err) throw (err);
            result.menu.category[obj.sec_n].item.push({'listing': obj.listing, 'price': obj.price});
            console.log(JSON.stringify(result, null, " "));
            JSONtoXML('beermenu.xml', result, function(err){
